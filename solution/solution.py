...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        """
        # NOTE: This is an ORDER-AWARE metric (same family as MAP / MRR / NDCG):
        # a relevant chunk sitting lower in the ranking is penalised, so reranking
        # relevant chunks toward the top raises the score without changing the set.
        expected_tokens = _tokenize(expected)
        if not expected_tokens:
            return 1.0
        if not contexts:
        ...
        # Step 1 — mark each chunk relevant/not by expected-token coverage.
        ...
        # Steps 2-3 — rank-aware Average Precision@K.
        hits = 0
        ap_sum = 0.0
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial judge. Score the AI answer against the rubric.\n"
            "Return ONLY a JSON object mapping each criterion to a score in [0, 1].\n\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n\n"
            f"Rubric criteria:\n{rubric_lines}\n"
        )
        ...
        try:
            ...
            for criterion in rubric:
                ...
                # Accept 1-5 rubric scale and normalise to 0-1.
                if value > 1.0:
                    value = value / 5.0
                scores[criterion] = max(0.0, min(1.0, value))
        except (ValueError, KeyError, TypeError, AttributeError):
            # A parse failure is a MEASUREMENT error, not a real 0.5 score.
            # Surface it via judge_failed so it can be counted, not silently
            # averaged into the results and skew the aggregate metrics.
            scores = {criterion: 0.5 for criterion in rubric}
            judge_failed = True

        return {"scores": scores, "reasoning": raw, "judge_failed": judge_failed}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        def _avg(score_dict: dict[str, Any]) -> float:
            values = list(score_dict.get("scores", {}).values())
        ...
        per_response = [_avg(s) for s in scores_batch]
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # More than one weak dimension → systemic problem.
        if len(low) > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures, start=1):
            ...
            suggested_fix = suggestions[i - 1] if i - 1 < len(suggestions) else "TBD"
            rows.append(
                f"| {failure_id} | {ftype} | {root_cause} | {suggested_fix} | Open |"
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            "hallucination": "Implement a hallucination checker to filter unsupported claims and tighten faithfulness guardrails",
            "irrelevant": "Improve prompt clarity and intent detection so answers address the question asked",
            "incomplete": "Increase chunk size / context window and add few-shot examples of complete answers",
            "off_topic": "Strengthen intent routing so queries reach the correct knowledge base",
            "refusal": "Relax over-strict guardrails so the agent answers in-scope questions",
        }

        suggestions: list[str] = []
        # Prioritise by frequency (cluster the biggest failure groups first).
        for ftype, _count in sorted(categories.items(), key=lambda kv: kv[1], reverse=True):
            if ftype in suggestion_map:
                suggestions.append(suggestion_map[ftype])

        # Generic fallbacks to guarantee at least 3 actionable suggestions.
        fallback = [
            "Expand the golden dataset with more edge cases and adversarial inputs",
            "Add automated regression checks to the CI/CD pipeline as a quality gate",
            "Calibrate the LLM-as-Judge against human ratings to reduce bias",
        ]
        for fb in fallback:
